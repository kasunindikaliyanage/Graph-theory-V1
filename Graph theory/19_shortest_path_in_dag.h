#ifndef simple_graph_20
#define simple_graph_20
...
class simple_graph {
	...
	friend void sortest_path_topological(simple_graph&, int);
...
void sortest_path_topological(simple_graph&graph, int source_vertex_id)
{
	...
	graph.initialize_and_find_vertex(source_vertex_id);

	// run shortest path on topologically sorted array
	// at this point grap.processed_list contains topologically sorted list of vertices
	for each (auto u in graph.processed_list)
	{
